...
describe('calendarFilter', () => {
  ...
  const items = [
    ...
    {} as Partial<Item>, // date is undefined
  ];

  it('filters IS_BEFORE_TODAY', () => {
    const result = calendarFilter<(typeof items)[0] & { date?: Date }, 'date'>(
      items,
      'date',
      CalendarOperEnum.IS_BEFORE_TODAY,
      today
    );
  ...
  it('should skip items where the date field is undefined', () => {
    const result = calendarFilter<(typeof items)[0] & { date?: Date }, 'date'>(
      items,
      'date',
      CalendarOperEnum.IS_TODAY,
      today
    );
    expect(result).toEqual([{ date: new Date('2023-12-28') }]);
  });

  it('filters IS_AFTER_TODAY', () => {
    const result = calendarFilter<(typeof items)[0] & { date?: Date }, 'date'>(
      items,
      'date',
      CalendarOperEnum.IS_AFTER_TODAY,
      today
    );
  ...
  it('filters IS_YESTERDAY', () => {
    const result = calendarFilter<(typeof items)[0] & { date?: Date }, 'date'>(
      items,
      'date',
      CalendarOperEnum.IS_YESTERDAY,
      today
    );
    expect(result.map((i) => i.date && i.date.toISOString())).toContain(new Date('2023-12-27').toISOString());
    expect(result).toHaveLength(1);
  ...
  it('filters IS_WEEKEND', () => {
    const result = calendarFilter<(typeof items)[0] & { date?: Date }, 'date'>(
      items,
      'date',
      CalendarOperEnum.IS_WEEKEND,
      today
    );
  ...
  it('filters IS_WEEKDAY', () => {
    const result = calendarFilter<(typeof items)[0] & { date?: Date }, 'date'>(
      items,
      'date',
      CalendarOperEnum.IS_WEEKDAY,
      today
    );
  ...
  it('filters IS_FUTURE', () => {
    const result = calendarFilter<(typeof items)[0] & { date?: Date }, 'date'>(
      items,
      'date',
      CalendarOperEnum.IS_FUTURE,
      today
    );
  ...
  it('throws for unsupported operator', () => {
    expect(() => calendarFilter(items, 'date', 'unsupported', today)).toThrow();
  });
});
